...
class RenderFigure():
    ...
    def render_body(self):
        try:
          mystr=""
          loc={"db":self.db,"session": self.session,"render_collection": self.render_collection,"params":self.params,"getparams": self.getparams}
    ...
    def render_collection(self, collection,partial,as_):
        ...
        for x in collection:
            for j in myview.split("<%="):
                ...
                k=j.split("%>")
                loc={"paspremier":paspremier,as_: x,"index":i,  "params": self.params,"render_collection":self.render_collection,"json":json}
                #print(dict(x))
                if k[0]:
